...
final class BLETransport: NSObject, @unchecked Sendable, Transport {
    ...
    // Optional capture of every (sent APDU, received response) pair
    // for later conformance-harness replay. Enable by setting
    // `capturing = true` before invoking the operation; read out via
    // `capturedRecords` after it completes.
    var capturing: Bool = false
    private(set) var capturedRecords: [(send: Data, sw: UInt16, recv: Data)] = []
    ...
    func exchange(apdu: Data) async throws -> ExchangeResponse {
        ...
        if capturing {
            capturedRecords.append((send: apdu, sw: sw, recv: data))
        }
